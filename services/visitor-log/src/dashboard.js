...
<section id="records" hidden><div class="card" id="summary"><div><span class="caption" id="period">最近 30 天 · 页面访问</span><span class="number" id="views">—</span></div><div><span class="caption">不同 IP 数</span><span class="number" id="ips">—</span></div></div>
...
export const dashboardScript = String.raw`(() => {
  ...
  async function load(append = false) {
    ...
    status("正在读取…")
    try {
      const response = await fetch("/api/visits" + (append && cursor ? "?before=" + cursor : ""), {
      ...
      el("ips").textContent = data.summary.ips.toLocaleString()
      el("period").textContent = "最近 " + data.retentionDays + " 天 · 页面访问"
