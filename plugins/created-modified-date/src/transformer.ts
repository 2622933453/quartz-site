...
// 本地补丁：@napi-rs/simple-git 只能取「最新提交时间」，没有取「首次提交时间」的接口。
// shell 调 git log 取文件首次被加入仓库（--diff-filter=A）的时间作为创建时间，
// 这样无需在每篇文章 frontmatter 里手写 created。线上构建发生在 commit 之后，故总能取到。
function gitFirstAddDate(repoWorkdir: string, relativePath: string): string | undefined {
  try {
    const out = execFileSync(
      "git",
      ["log", "--follow", "--diff-filter=A", "--format=%aI", "--", relativePath],
      { cwd: repoWorkdir, encoding: "utf-8" },
    ).trim();
    if (!out) return undefined;
    const lines = out.split("\n");
    return lines[lines.length - 1]; // 最后一行 = 最早的一次「新增」
  } catch {
    return undefined;
  }
}
            ...
            for (const source of opts.priority) {
              if (source === "filesystem") {
                ...
                try {
                  ...
                  created ||= gitFirstAddDate(repositoryWorkdir, relativePath);
                } catch {
                  console.log(
                    styleText(
                      "yellow",
